...
<body>
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" id="login-form">
                        ...
                        <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                            Don’t have an account yet? <a href="{{url('/registrasi')}}" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign up</a>
                        </p>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        document.getElementById('login-form').addEventListener('submit', function(e) {
            ...
            .then(response => {
                const token = response.data.token; // Sesuaikan dengan respons dari login
                localStorage.setItem('authToken', token); // Simpan token di localStorage
                window.location.href = '/products'; // Arahkan pengguna ke halaman produk
            })
            .catch(error => {
        ...
        checkLoginStatus(); // Panggil fungsi checkLoginStatus saat halaman diload
    </script>
</body>
</html>
